...
export default function SocialWorkPage() {
  return (
    <RoleGuard
      ...
      fallbackTitle="Medical Social Work & SDOH Studio"
      fallbackMessage="Access to Social Determinants of Health (SDOH) profiles and community resource navigation is restricted to licensed medical social workers, care coordinators, and clinical case management teams."
    >
      <SocialWorkContent />
    </RoleGuard>
  );
...
function SocialWorkContent() {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      {/* Header */}
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 bg-gradient-to-r from-slate-900 via-slate-900/90 to-orange-950/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-orange-500/20 text-orange-300 border border-orange-500/30 text-xs font-bold uppercase">
              Medical Social Work & SDOH Hub
            </span>
          ...
          <p className="text-xs text-slate-300 mt-0.5">
            Identify social risk factors, connect patients to government aid & SNAP, and coordinate caregiver support.
          </p>
        </div>
        ...
        <select
          ...
          className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-orange-500"
        >
          {patients.map((p) => (
