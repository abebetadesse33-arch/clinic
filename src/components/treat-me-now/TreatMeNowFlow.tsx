...
interface TreatMeNowFlowProps {
  /** URL to redirect back to after login if user is not authenticated */
  returnPath?: string;
  /** Back link shown in breadcrumb */
  backHref?: string;
  backLabel?: string;
}

function TreatMeNowFlowInner({ returnPath = "/services/virtual-urgent-care/triage" }: TreatMeNowFlowProps) {
  ...
  // Restore from URL params (deep link from notification or queue re-entry)
  useEffect(() => {
  ...
  // Step 1 → Step 2: Start assessment
  const handleStartConcern = (concernLabel?: string) => {
    if (!isAuthenticated) {
      router.push(`/signin?redirect=${encodeURIComponent(returnPath)}`);
      return;
    }
    if (concernLabel) setSelectedInitialConcern(concernLabel);
    setStep("assessment");
  };

  // Step 2 → Step 3: Assessment submitted → AI Triage & Provider matching
  ...
  // Step 4: Confirm visit → finalize case + queue
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0B3B32] via-[#005C4B] to-[#04362D] p-8 sm:p-12 text-white shadow-2xl border border-teal-500/30 space-y-6">
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                ...
                >
                  ...
                  <span>Start Rapid Triage &amp; Connect</span>
...
/**
 * TreatMeNowFlow — the complete 5-step Treat Me Now™ urgent care flow.
 * Wraps content in Suspense for useSearchParams compatibility.
 */
export default function TreatMeNowFlow(props: TreatMeNowFlowProps) {
  return (
    <Suspense
      fallback={
